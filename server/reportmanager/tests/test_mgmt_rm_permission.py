"""Tests for ReportManager rm_permission management command
...
def test_rm_perms_from_empty():
    ...
    call_command("rm_permission", "test", "view_reportmanager")
...
def test_one_perm():
    user = User.objects.create_user("test", "test@example.com", "test")
    call_command("rm_permission", "test", "view_reportmanager")
    assert set(user.get_all_permissions()) == {"reportmanager.reportmanager_all"}
