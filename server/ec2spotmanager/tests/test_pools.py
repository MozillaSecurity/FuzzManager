...
def test_pools_view_pools(client):
    """Create pool and see that it is shown."""
    configs = (create_config(name="config #1"), create_config(name="config #2"))
...
def test_view_pool_view_simple_get(client):
    ...
    client.login(username="test", password="test")
    response = client.get(
        reverse("ec2spotmanager:poolview", kwargs={"poolid": pool.pk})
    )
    LOG.debug(response)
    assert response.status_code == requests.codes["ok"]
...
def test_pool_prices_view_simple_get(client):
    """No errors are thrown in template"""
    cfg = create_config(name="config #1", ec2_instance_types=["c4.2xlarge"])
    pool = create_pool(config=cfg)
    client.login(username="test", password="test")
    response = client.get(
        reverse("ec2spotmanager:poolprices", kwargs={"poolid": pool.pk})
    )
    LOG.debug(response)
    assert response.status_code == requests.codes["ok"]
...
def test_enable_pool_view_simple_get(client):
    ...
    client.login(username="test", password="test")
    response = client.get(
        reverse("ec2spotmanager:poolenable", kwargs={"poolid": pool.pk})
    )
    LOG.debug(response)
    assert response.status_code == requests.codes["ok"]
...
def test_disable_pool_view_simple_get(client):
    ...
    client.login(username="test", password="test")
    response = client.get(
        reverse("ec2spotmanager:pooldisable", kwargs={"poolid": pool.pk})
    )
    LOG.debug(response)
    assert response.status_code == requests.codes["ok"]
...
def test_cycle_pool_view_simple_get(client):
    ...
    client.login(username="test", password="test")
    response = client.get(
        reverse("ec2spotmanager:poolcycle", kwargs={"poolid": pool.pk})
    )
    LOG.debug(response)
    assert response.status_code == requests.codes["ok"]
...
def test_delete_pool_message_view_simple_get(client):
    ...
    client.login(username="test", password="test")
    response = client.get(
        reverse("ec2spotmanager:poolmsgdel", kwargs={"msgid": msg.pk})
    )
    LOG.debug(response)
    assert response.status_code == requests.codes["ok"]
