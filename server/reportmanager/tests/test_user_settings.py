...
from reportmanager.models import Tool, User

LOG = logging.getLogger("fm.reportmanager.tests.usersettings")
pytestmark = pytest.mark.usefixtures("reportmanager_test")


def test_user_settings_no_login(client):
    """Request without login hits the login redirect"""
    path = reverse("reportmanager:usersettings")
...
def test_user_settings_edit(client, cm):
    ...
    response = client.post(
        reverse("reportmanager:usersettings"),
        data={
    ...
    assert response.url == "/reportmanager/usersettings/"
