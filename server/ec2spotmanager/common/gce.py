...
INSTANCE_TYPES = tuple(
    [
        InstanceType(f"n1-standard-{cores}", cores, cores * 3.75)
        for cores in (1, 2, 4, 8, 16, 32, 64, 96)
    ]
    + [
        InstanceType(f"n1-highmem-{cores}", cores, cores * 6.5)
        for cores in (2, 4, 8, 16, 32, 64, 96)
    ]
    + [
        InstanceType(f"n1-highcpu-{cores}", cores, cores * 0.9)
        for cores in (2, 4, 8, 16, 32, 64, 96)
    ]
    + [InstanceType("f1-micro", 0.2, 0.6)]
    + [InstanceType("g1-small", 0.5, 1.7)]
    + [
        InstanceType(f"n1-ultramem-{cores}", cores, cores * 24.025)
