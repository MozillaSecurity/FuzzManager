...
class EC2SpotCloudProvider(CloudProvider):
    ...
    def terminate_instances(self, instances_ids_by_region):
        for region, instance_ids in instances_ids_by_region.items():
            ...
            self.logger.info(
                "Terminating %s instances in region %s", len(instance_ids), region
            )
            ...
            for boto_instance in boto_instances:
                ...
                if not (
                    (boto_instance.id in instance_ids)
                    or (
                    ...
                    self.logger.error(
                        "Instance with EC2 ID %s (status %d) is not in region list for "
                        "region %s",
                        boto_instance.id,
                        state_code,
                        region,
    ...
    def cancel_requests(self, requested_instances_by_region):
        ...
        self.logger.info(
            "Canceling %s requests in region %s", len(instance_ids), region
        )
        ...
        try:
            ...
            if code_match is not None:
                ...
                if code == "MaxSpotInstanceCountExceeded":
                    self.logger.warning(
                        "start_instances: Maximum instance count exceeded for region "
                        "%s",
                        region,
                    )
    ...
    def check_instances_requests(self, region, instances, tags):
        ...
        for req_id, result in zip(instances, results):
            if isinstance(result, boto.ec2.instance.Instance):
                ...
                self.logger.info(
                    "Spot request fulfilled %s -> %s (status: %s)",
                    req_id,
                    result.id,
                    status_desc,
                )
                ...
                # updatable so our update code can pick it up and update it accordingly
            ...
            # request object is returned in case request is closed/cancelled/failed
                ...
                if result.state in {"cancelled", "closed"}:
                    ...
                    # type/zone for a while
                    # XXX: in some cases we are the ones who cancelled the request, if
                    # so we should not blacklist
                    self.logger.info(
                ...
                self.logger.warning(
                    "Spot request %s returned %s", req_id, type(result).__name__
                )
        ...
        try:
            if pool_id is None:
                boto_instances = cluster.find(
                    filters={"tag-key": SPOTMGR_TAG + "-PoolId"}
                )
            else:
                boto_instances = cluster.find(
                    filters={"tag:" + SPOTMGR_TAG + "-PoolId": str(pool_id)}
                )
        except boto.exception.BotoServerError as msg:
            code_match = re.search(r"<Code>([A-Za-z]+)</Code>", str(msg))
            if code_match is not None:
            ...
            for result in paginator.paginate(**spot_history_args):
                for price in result["SpotPriceHistory"]:
                    if price["AvailabilityZone"] in zone_blacklist:
                        continue
                    (
                        prices.setdefault(price["InstanceType"], {})
