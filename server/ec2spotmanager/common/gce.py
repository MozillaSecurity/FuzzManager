...
INSTANCE_TYPES = tuple(
    [
        InstanceType("n1-standard-%d" % (cores,), cores, cores * 3.75)
        for cores in (1, 2, 4, 8, 16, 32, 64, 96)
    ]
    + [
        InstanceType("n1-highmem-%d" % (cores,), cores, cores * 6.5)
        for cores in (2, 4, 8, 16, 32, 64, 96)
    ]
    + [
        InstanceType("n1-highcpu-%d" % (cores,), cores, cores * 0.9)
        for cores in (2, 4, 8, 16, 32, 64, 96)
    ]
    + [InstanceType("f1-micro", 0.2, 0.6)]
    + [InstanceType("g1-small", 0.5, 1.7)]
    + [
        InstanceType("n1-ultramem-%d" % (cores,), cores, cores * 24.025)
        for cores in (40, 80, 160)
    ]
