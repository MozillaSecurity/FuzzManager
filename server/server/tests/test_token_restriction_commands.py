...
class TestManageTokenRestrictionsCommand:
    ...
    def test_add_restriction(self, setup_token):
        ...
        stdout = io.StringIO()
        call_command('manage_token_restrictions', token.key, '--add=10.0.0.0/8', '--force', stdout=stdout)
        
        # Verify the restriction was added
        assert TokenIPRestriction.objects.filter(token=token, ip_range="10.0.0.0/8").exists()
    ...
    def test_add_restriction_with_open_policy(self, setup_token):
        ...
        with patch('builtins.input', return_value='y'):
            stdout = io.StringIO()
            call_command('manage_token_restrictions', token.key, '--add=10.0.0.0/8', stdout=stdout)
            
            # Verify the open policy was removed and new restriction added
            assert not TokenIPRestriction.objects.filter(token=token, ip_range="0.0.0.0/0").exists()
            assert TokenIPRestriction.objects.filter(token=token, ip_range="10.0.0.0/8").exists()
    ...
    def test_remove_restriction(self, setup_token):
        ...
        stdout = io.StringIO()
        call_command('manage_token_restrictions', token.key, '--remove=10.0.0.0/8', '--force', stdout=stdout)
        
        # Verify the restriction was removed
        assert not TokenIPRestriction.objects.filter(token=token, ip_range="10.0.0.0/8").exists()
    ...
    def test_remove_last_restriction_with_confirmation(self, setup_token):
        ...
        with patch('builtins.input', return_value='y'):
            stdout = io.StringIO()
            call_command('manage_token_restrictions', token.key, '--remove=10.0.0.0/8', stdout=stdout)
    ...
    def test_reset_to_open_policy(self, setup_token):
        ...
        with patch('builtins.input', return_value='y'):
            stdout = io.StringIO()
            call_command('manage_token_restrictions', token.key, '--reset', stdout=stdout)
            
            # Verify the old restrictions were removed and default ones added
            assert not TokenIPRestriction.objects.filter(token=token, ip_range="10.0.0.0/8").exists()
            assert not TokenIPRestriction.objects.filter(token=token, ip_range="192.168.1.0/24").exists()
            assert TokenIPRestriction.objects.filter(token=token, ip_range="0.0.0.0/0").exists()
            assert TokenIPRestriction.objects.filter(token=token, ip_range="::/0").exists()
