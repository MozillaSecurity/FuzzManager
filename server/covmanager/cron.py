...
def identify_coverage_drops(revision, ipc_only=False):
    ...
    if ipc_only:
        recent_collections = (
            Collection.objects.filter(description__startswith="Weekly Report")
            .filter(description__contains="IPC")
            .order_by("-created")[:2]
        )
    else:
        recent_collections = (
            Collection.objects.filter(description__startswith="Weekly Report")
            .exclude(description__contains="IPC")
            .order_by("-created")[:2]
        )
    ...
    current, previous = recent_collections
    ...
    # ToDo: Do I need to refresh recent_collections before accessing the summary?
