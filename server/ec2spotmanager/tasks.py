...
def _determine_best_location(config, count, cache=None):
    ...
    for provider in PROVIDERS:
        ...
        if ProviderStatusEntry.objects.filter(
            provider=provider, isCritical=True
        ).exists():
            continue
        ...
        for instance_type in cloud_provider.get_instance_types(config):
            ...
            # keep track of all instance types with the least number of cores for this
            # config
        ...
        # replace the allowed instance types with those that are <= count, or the
        # smallest if none are
            ...
            for region in data:
                ...
                for zone in data[region]:
                    ...
                    if (
                        ...
                        logger.debug(
                            "%s/%s/%s/%s is blacklisted",
                            cloud_provider.get_name(),
                            region,
                            zone,
                            instance_type,
                        )
                    ...
                    prices = [
                        price / cores_per_instance[instance_type]
                        for price in data[region][zone]
                    ]
                    ...
                    if prices[0] > cloud_provider.get_max_price(config):
                        rejected_prices[zone] = min(
                            rejected_prices.get(zone, 9999), prices[0]
                        )
                        continue

                    median = get_price_median(prices)
                    if best_median is None or median <= best_median:
                        # don't care about excluding stopped/stopping, as we just want
                        # to know how "busy" the zone is
                        instances = int(
                        ...
                        logger.debug(
                            "Best price median currently %r in %s/%s (%s, %d "
                            "instances)",
                            best_median,
                            best_region,
                            best_zone,
...
def _start_pool_instances(pool, config, count=1):
    ...
    try:
        ...
        (
            ...
            instance_type,
            rejected_prices,
        ) = _determine_best_location(config, count, cache=cache)
        ...
        #
        # if we have chosen the smallest possible instance that will put us over the
        #   requested core count, we will only be spawning 1 instance
        #
        # otherwise there may be a remainder if this is not an even division. let that
        #   be handled in the next tick so that the next smallest instance will be
        #   considered
        #
        ...
        #
        #     -> we will only request 1x 8-core instance this time around, leaving the
        #        required count at 4
        ...
        if provider == "EC2Spot":
            ...
            if not userdata:
                ...
                _update_pool_status(
                    pool,
                    "config-error",
                    "Configuration error: Failed to compile userdata",
                )
                return
        ...
        requested_instances = cloud_provider.start_instances(
            config, region, zone, userdata, image, instance_type, count, tags
        )
...
def _update_provider_status(provider, type_, message):
    ...
    if not ProviderStatusEntry.objects.filter(
        type=POOL_STATUS_ENTRY_TYPE[type_],
        provider=provider,
        msg=message,
        ...
        entry.save()
        logger.error(
            "Logging ProviderStatusEntry(%s): %s (critical=%r)",
            provider,
            message,
            is_critical,
        )
...
def _update_pool_status(pool, type_, message):
    ...
    if not PoolStatusEntry.objects.filter(
        ...
        entry.save()
        logger.error(
            "Logging PoolStatusEntry(%d): %s (critical=%r)",
            pool.pk,
            message,
            is_critical,
        )
    ...
    try:
        ...
        if requested:
            ...
            (
                successful_requests,
                failed_requests,
            ) = cloud_provider.check_instances_requests(region, list(requested), tags)

            for req_id in successful_requests:
                instance = requested[req_id]
                instance.created = (
                    timezone.now()
                )  # reset creation time now that the instance really exists
            ...
            for req_id in failed_requests:
                ...
                if failed_requests[req_id]["action"] == "blacklist":
                    ...
                    instance.delete()
                    _update_pool_status(
                        pool, "temporary-failure", failed_requests[req_id]["reason"]
                    )
        ...
        if instances_created:
            ...
            PoolStatusEntry.objects.filter(
                pool=pool,
                type__in=(
                    POOL_STATUS_ENTRY_TYPE["max-spot-instance-count-exceeded"],
                    POOL_STATUS_ENTRY_TYPE["temporary-failure"],
                ),
            ).delete()
    ...
    try:
        ...
        for cloud_id, cloud_data in cloud_instances.items():
            ...
            if (
                ...
                logger.warning(
                    "see: https://github.com/MozillaSecurity/FuzzManager"
                    "/pull/550#discussion_r284260225"
                )

                # The instance is not marked as updatable. We must not touch it because
                # a spawning thread is still managing this instance. However, we must
                # also remove this instance from the instances_left list if it's already
            ...
            # Whenever we see an instance that is not in our instance list for that
            # region, make sure it's a terminated instance because we should never have
            # a running instance that matches the search above but is not in our
            # database.
            if cloud_id not in instances:
                if cloud_data["status"] not in {
                    ...
                    # will not be in our instances list but returned by provider. In
                    # this case, we try to load it directly from the database.
                    q = Instance.objects.filter(instance_id=cloud_id)
                    if q:
                        instance = q[0]
                        logger.error(
                            "[Pool %d] Instance with ID %s was reloaded from database.",
                            q.pool_id,
                            cloud_id,
                        )
                    else:
                        logger.error(
                            "[Pool ?] Instance with ID %s is not in database", cloud_id
                        )

                        # Terminate at this point, we run in an inconsistent state
                        raise RuntimeError(
                            "Database and cloud provider are inconsistent"
                        )
        ...
        for instance in instances_left:
            ...
            if instance.instance_id not in debug_cloud_instance_ids_seen:
                if timezone.now() - instance.created < timezone.timedelta(minutes=5):
                    logger.warning(
                        "[Pool %d] Machine %s has no corresponding machine in %s/%s, "
            ...
            logger.info(
                "[Pool %d] Deleting instance with cloud instance ID %s from our "
                "database: %s",
                instance.pool_id,
                instance.instance_id,
                ", ".join(reasons),
...
def cycle_and_terminate_disabled(provider, region):
    ...
    provider_has_critical_error = ProviderStatusEntry.objects.filter(
        provider=provider, isCritical=True
    ).exists()
    ...
    try:
        ...
        for instance in Instance.objects.filter(provider=provider, region=region):
            if instance.pool_id not in pool_disable:
                ...
                elif PoolStatusEntry.objects.filter(
                    pool=pool, isCritical=True
                ).exists():
                    pool_disable[instance.pool_id] = "Pool error"
                elif not pool.isEnabled:
                    pool_disable[instance.pool_id] = "Disabled"
                elif (
                    pool.last_cycled is None
                    or pool.last_cycled
                    + timezone.timedelta(seconds=pool.config.flatten().cycle_interval)
                    < timezone.now()
                ):
            ...
            if pool_disable[instance.pool_id]:
                if instance.status_code not in {
                    INSTANCE_STATE["shutting-down"],
                    INSTANCE_STATE["terminated"],
                }:
                ...
                elif instance.status_code not in {
                    INSTANCE_STATE["shutting-down"],
                    INSTANCE_STATE["terminated"],
                }:
        ...
        for pool_id, count in instances_by_pool.items():
            logger.info(
                "[Pool %d] %s, terminating %d instances in %s/%s...",
                pool_id,
                pool_disable[pool_id],
                count,
                provider,
                region,
            )
        if requests_to_terminate:
            _terminate_instance_request_ids.delay(
                provider, region, requests_to_terminate
            )
...
def check_and_resize_pool(pool_id):
    ...
    try:
        ...
        if (
            pool.last_cycled is None
            or pool.last_cycled + timezone.timedelta(seconds=config.cycle_interval)
            < timezone.now()
        ):
        ...
        for instance in instances:
            if instance.status_code in [
                ...
                logger.info(
                    "[Pool %d] Deleting terminated instance with ID %s from our "
                    "database.",
                    pool_id,
                    instance.instance_id,
                )
        ...
        if instance_cores_missing > 0:
            logger.info(
                "[Pool %d] Needs %s more instance cores, starting...",
                pool_id,
                instance_cores_missing,
            )
            ...
            for instance in Instance.objects.filter(pool=pool).order_by("created"):
                if instance_cores_missing + instance.size > 0:
                    # If this instance would leave us short of cores, let it run.
                    # Otherwise the pool size may oscillate.
                    continue
            ...
            if instances:
                ...
                logger.info(
                    "[Pool %d] Has %d instance cores over limit in %d instances, "
                    "queuing for termination...",
                    pool_id,
                    instance_cores_missing,
                    len(instances),
                )
...
def terminate_instances(pool_instances):
    ...
    for instance in Instance.objects.filter(
        pk__in=itertools.chain.from_iterable(pool_instances)
    ):
        requested, running = instances_by_provider_region.setdefault(
    ...
    for (provider, region), (
        requested,
        running,
    ) in instances_by_provider_region.items():
        if requested:
            provider_parallel.append(
                _terminate_instance_request_ids.si(provider, region, requested)
            )
        if running:
            provider_parallel.append(
                _terminate_instance_ids.si(provider, region, running)
            )
