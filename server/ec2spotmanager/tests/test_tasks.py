...
def test_create_instance(mocker):
    ...
    config = create_config(
        ...
        ec2_image_name="os/2",
        ec2_allowed_regions=["redmond", "toronto"],
        max_price="0.1",
    ...
    assert {call[0] for call in cluster.method_calls} == {
        "connect",
...
def test_fulfilled_spot_instance(mocker):
    ...
    pool = create_pool(config=config, enabled=True)
    orig = create_instance(
        None,
        pool=pool,
        status_code=INSTANCE_STATE["requested"],
        ec2_instance_id="req123",
        ec2_region="redmond",
        ec2_zone="mshq",
    )
...
def test_spot_instance_blacklist(mocker):
    ...
    with pytest.raises(
        UncatchableException,
        match="No allowed region was cheap enough to spawn instances",
    ):
        check_and_resize_pool(pool.pk)
...
def test_pool_trim():
    ...
    pool = create_pool(
        config=config,
        last_cycled=timezone.now() - datetime.timedelta(seconds=300),
        enabled=True,
    )
    create_instance(
        None,
...
@pytest.mark.parametrize(
    ...
    [
        (_terminate_instance_ids, "terminate_instances"),
