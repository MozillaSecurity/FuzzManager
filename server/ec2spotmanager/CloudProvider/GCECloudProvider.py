...
#
# Currently laniakea is using libcloud as the backend, which means operations are
# synchronous.  API access is not very optimal as a result. Launching typically takes
# ~30s and terminating ~80s.
#
...
class GCECloudProvider(CloudProvider):
            ...
            for retry in retries:
                try:
                    ...
                    if retry is None:
                        raise
                    self.logger.warning(
                        "Connect error: %s, retrying in %d seconds", error, retry
                    )
                    time.sleep(retry)
        ...
        for region, instance_ids in instances_ids_by_region.items():
            ...
            for node in nodes:
                if not (node.name in instance_ids or node.state == "STOPPED"):
                    self.logger.error(
                        "Instance with GCE Name %s (status %s) is not in node list",
                        node.name,
                        node.state,
                    )
    ...
    def check_instances_requests(self, region, instances, tags):
        ...
        for node in libcloud_nodes:
            ...
            self.logger.info(
                "Instance %s (%s) is %s (%s)",
                node.name,
                requests[node.name]["hostname"],
                node.extra["status"],
                INSTANCE_STATE_CODE[requests[node.name]["status_code"]],
            )
            # Now that we saved the object into our database, mark the instance as
            # updatable so our update code can pick it up and update it accordingly when
            # it changes states
            # TODO: This should use a laniakea API if issue laniakea issue #40 is fixed.
            #       (if it isn't removed)
            tags = _LowercaseDict(tags)
            tags[SPOTMGR_TAG + "-Updatable"] = "1"
    ...
    def check_instances_state(self, pool_id, region):
        ...
        try:
            if pool_id is None:
                libcloud_nodes = (
                    cluster.filter()
                    .has_labels([(SPOTMGR_TAG + "-PoolId").lower()])
                    .nodes
                )
            else:
                libcloud_nodes = (
                    cluster.filter()
                    .labels({(SPOTMGR_TAG + "-PoolId").lower(): str(pool_id)})
                    .nodes
                )
    ...
    def get_prices_per_region(self, region_name, instance_types=None):
        ...
        # cases we do get details like "price in Tokyo" and "price in Japan" (both
        # asia-northeast1), but we don't know which sub-zone those are, so we take the
            ...
            if (
                ...
                if sku["category"]["resourceGroup"] == "CPU":
                    if sku["description"].startswith(
                        "Preemptible Memory-optimized Instance Core"
                    ):
                        what = "memopt-cpu"
                elif sku["category"]["resourceGroup"] == "F1Micro":
                    if sku["description"].startswith(
                        "Preemptible Micro Instance with burstable CPU"
                    ):
                        what = "f1-inst"
                elif sku["category"]["resourceGroup"] == "G1Small":
                    if sku["description"].startswith(
                        "Preemptible Small Instance with 1 VCPU"
                    ):
                        what = "g1-inst"
                    ...
                    if sku["description"].startswith(
                        ...
                        what = "n1-ram"
                    elif sku["description"].startswith(
                ...
                for region in sku["serviceRegions"]:
                    ...
                    # TODO: the friendly description describes the zone, but I have no
                    #       way of mapping that to an API zone
                    #       (eg. Tokyo & Japan -> asia-northeast1-a & asia-northeast1-b)
        ...
        for instance_type, cores in CORES_PER_INSTANCE.items():
            ...
            for zone, prices in zones.items():
                if any(key not in prices for key in keys.values()):
                    continue
                region = result.setdefault(instance_type, {region_name: {}})[
                    region_name
                ]
                assert zone not in region
                region[zone] = [
                    max(prices.get(keys.get("cpu"), [0])) * cores
        ...
        for instance_type, all_regions in result.items():
            for region, region_data in all_regions.items():
                ...
                all_zones_region = all_zones_result[instance_type].setdefault(
                    region, {}
                )
