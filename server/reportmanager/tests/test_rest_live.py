...
@pytest.mark.skip
def test_RESTReportEntryInterface(live_server, fm_user):
    url = urlsplit(live_server.url)
    url = f"{url.scheme}://{url.hostname}:{url.port}/reportmanager/rest/reports/"
...
def test_RESTSignatureInterface(live_server):
    url = urlsplit(live_server.url)
    url = f"{url.scheme}://{url.hostname}:{url.port}/reportmanager/rest/signatures/"
