...
Represents a report signature as specified in
https://wiki.mozilla.org/Security/ReportSignatures
...
class ReportSignature:
    def __init__(self, rawSignature):
        ...
        @param rawSignature: A JSON-formatted string representing the report signature
        """
    ...
    def fromFile(signatureFile):
        with open(signatureFile) as sigFd:
            return ReportSignature(sigFd.read())
    ...
    def matches(self, reportInfo):
        ...
        if (
            ...
            and reportInfo.configuration.platform not in self.platforms
        ):
            return False

        if (
            self.operatingSystems is not None
            and reportInfo.configuration.os not in self.operatingSystems
        ):
            return False

        if (
            self.products is not None
            and reportInfo.configuration.product not in self.products
        ):
            return False
        ...
        for symptom in self.symptoms:
            ...
            if not symptom.matches(reportInfo):
                return False
    ...
    def matchRequiresTest(self):
        ...
        This method can be used to avoid attaching a testcase to the reportInfo
    ...
    def getRequiredOutputSources(self):
        ...
        @rtype: list(str)
        @return: A list of output identifiers (e.g. stdout, stderr or reportdata)
        ...
        for symptom in self.symptoms:
            if isinstance(symptom, OutputSymptom):
                if symptom.src is None:
                    ...
                    return ["stdout", "stderr", "reportdata"]
                ret.append(symptom.src)

        return ret

    def getDistance(self, reportInfo):
        distance = 0

        for symptom in self.symptoms:
            if isinstance(symptom, StackFramesSymptom):
                symptomDistance = symptom.diff(reportInfo)[0]
    ...
    def fit(self, reportInfo):
        ...
        symptomsDiff = self.getSymptomsDiff(reportInfo)
    ...
    def getSymptomsDiff(self, reportInfo):
        symptomsDiff = []
        for symptom in self.symptoms:
            if symptom.matches(reportInfo):
                ...
                if isinstance(symptom, StackFramesSymptom):
                    proposedSymptom = symptom.diff(reportInfo)[1]
                    if proposedSymptom:
                        symptomsDiff.append(
                            {
                                "offending": True,
    ...
    def getSignatureUnifiedDiffTuples(self, reportInfo):
        diffTuples = []
        ...
        newLines = []
        newRawReportSignature = self.fit(reportInfo)
        if newRawReportSignature:
            newLines = newRawReportSignature.rawSignature.splitlines()
