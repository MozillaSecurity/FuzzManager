...
class ReportEntrySerializer(serializers.ModelSerializer):
    ...
    product_version = serializers.CharField(
        source="product.version", max_length=127, required=False, allow_blank=True
    )
    ...
    testcase_size = serializers.IntegerField(
        source="testcase.size", required=False, default=0
    )
    testcase_quality = serializers.IntegerField(
        source="testcase.quality", required=False, default=0
    )
    testcase_isbinary = serializers.BooleanField(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not include_raw:
            for field_name in ("rawReportData", "rawStdout", "rawStderr"):
    ...
    class Meta:
        ...
        ordering = ["-id"]
        read_only_fields = ("bucket", "id", "shortSignature", "reportAddress")
    ...
    def create(self, attrs):
        ...
        """
        missing_keys = {"rawStdout", "rawStderr", "rawReportData"} - set(attrs.keys())
        if missing_keys:
        ...
        # Parse the incoming data using the report signature package from FTB
        ...
        # Populate certain fields here from the ReportInfo object we just got
        if reportInfo.reportAddress is not None:
            attrs["reportAddress"] = f"0x{reportInfo.reportAddress:x}"
        attrs["shortSignature"] = reportInfo.createShortSignature()
...
class BucketVueSerializer(BucketSerializer):
    ...
    def get_opt_pre_url(self, sig):
        return reverse("reportmanager:sigoptpre", kwargs={"sigid": sig.id})

    def get_view_url(self, sig):
        return reverse("reportmanager:sigview", kwargs={"sigid": sig.id})
...
class ReportEntryVueSerializer(ReportEntrySerializer):
    ...
    def get_view_url(self, entry):
        return reverse("reportmanager:reportview", kwargs={"reportid": entry.id})

    def get_sig_view_url(self, entry):
        if entry.bucket:
            return reverse("reportmanager:sigview", kwargs={"sigid": entry.bucket.id})
        return None

    def get_sig_new_url(self, entry):
        return f"{reverse('reportmanager:signew')}?reportid={entry.id}"

    def get_find_sigs_url(self, entry):
        return reverse("reportmanager:findsigs", kwargs={"reportid": entry.id})
...
class NotificationSerializer(serializers.ModelSerializer):
    ...
    def get_actor_url(self, notification):
        if isinstance(notification.actor, Bucket):
            return reverse(
                "reportmanager:sigview", kwargs={"sigid": notification.actor.id}
            )
        return None
    ...
    def get_target_url(self, notification):
        if isinstance(notification.target, ReportEntry):
            return reverse(
                "reportmanager:reportview", kwargs={"reportid": notification.target.id}
            )
        return None
    ...
    def get_external_bug_url(self, notification):
        if isinstance(notification.target, Bug):
            return (
                ...
                f"/{notification.target.externalId}"
            )
        return None
